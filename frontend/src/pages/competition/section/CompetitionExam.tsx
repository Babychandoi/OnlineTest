...
import { getCompetitionExam, resultCompetition } from '../../../services/service';
import { CompetitionExamDetail, CompetitionResponse } from '../../../types/competition';


const CompetitionExam: React.FC = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const competitionId = searchParams.get('competitionId');

  const [competition, setCompetition] = useState<CompetitionResponse | null>(null);
  const [exam, setExam] = useState<CompetitionExamDetail | null>(null);
  ...
  const calculateRemainingTime = useCallback((startTime: string, durationMinutes: number) => {
  ...
  const handleBackToSubjects = useCallback(() => {
    ...
    const confirmLeave = window.confirm('Bạn có chắc muốn thoát? Dữ liệu bài thi sẽ không được lưu.');
    if (confirmLeave) {
      navigate('/quiz-test');
    }
  }, [navigate, examResult]);
  ...
  const handleSubmitExam = useCallback(async () => {
    ...
    try {
      const payload = {
        examId: competition.competitionExam.id,
      ...
      const response = await resultCompetition(payload,competition.id);
  ...
  }, [exam, competition, answeredCount, showSubmitConfirm, answers]);
  ...
  useEffect(() => {
    ...
    const fetchExamDetail = async () => {
      try {
        setLoading(true);
        const response = await getCompetitionExam(competitionId);
        if (response.code === 200) {
          setCompetition(response.data);
          setExam(response.data.competitionExam);
          
          // Calculate remaining time based on server startTime
          const remaining = calculateRemainingTime(
            response.data.startTime, 
            response.data.competitionExam.duration
          );
          ...
          setExamStartTime(new Date(response.data.startTime).getTime());
        } else {
    ...
    fetchExamDetail();
  }, [competitionId, navigate, calculateRemainingTime]);

  // Timer - recalculate from server time every second
  useEffect(() => {
    if (!exam || !competition || !examStartTime || isExamSubmitted) return;
  ...
  }, [exam, competition, examStartTime, handleAutoSubmit, isExamSubmitted, calculateRemainingTime]);
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      e.preventDefault();
      e.returnValue = 'Bạn có chắc muốn rời khỏi trang? Dữ liệu sẽ không được lưu.';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 sticky top-0 z-10 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-1">{competition.title}</h1>
              <div className="flex items-center gap-3 flex-wrap">
                <Badge color="purple">{exam.type === 'FREE' ? 'Miễn phí' : 'Có phí'}</Badge>
                <Badge color="blue">{competition.subjectName}</Badge>
                <Badge color="green">{competition.gradeName}</Badge>
...
export default CompetitionExam;